    ...
    <div class="sidebar shadow-sm border border-gray-100 p-3 flex flex-col" style="width: 250px; height: 100vh; position: fixed; top: 0; left: 0;">
        ...
        <div class="flex items-center gap-3 mb-3">
            ...
            <div>
                <h4 class="text-xl font-bold text-gray-900 leading-tight">
                    {{ request()->is('tenant*') ? 'Tenant Portal' : 'Manager' }}
                </h4>
        ...
        <nav class="flex-1 px-2 py-2 space-y-1">
            ...
            @if(request()->is('tenant*'))
                ...
                {{ request()->is('tenant/dashboard') ? 'bg-blue-100 text-blue-600' : '!text-gray-900 hover:bg-blue-50 hover:!text-blue-600' }}">
                    <x-heroicon-o-home class="w-5 h-5 shrink-0" /> Dashboard
                </a>

                <a href="/tenant/payments"
                class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition !no-underline hover:!no-underline 
                {{ request()->is('tenant/payments*') ? 'bg-blue-100 text-blue-600' : '!text-gray-900 hover:bg-blue-50 hover:!text-blue-600' }}">
                    <x-heroicon-o-currency-dollar class="w-5 h-5 shrink-0" /> Payments
                </a>

                <a href="/tenant/maintenance"
                class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition !no-underline hover:!no-underline 
                {{ request()->is('tenant/maintenance*') ? 'bg-blue-100 text-blue-600' : '!text-gray-900 hover:bg-blue-50 hover:!text-blue-600' }}">
                    <x-heroicon-o-wrench-screwdriver class="w-5 h-5 shrink-0" /> Maintenance
                </a>
            ...
            @else
                ...
                class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition !no-underline hover:!no-underline 
                {{ request()->is('admin/dashboard') ? 'bg-blue-100 text-blue-600' : '!text-gray-900 hover:bg-blue-50 hover:!text-blue-600' }}">
                    <x-heroicon-o-building-office-2 class="w-5 h-5 shrink-0" /> Dashboard
                </a>

                <a href="/admin/properties/create"
                class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition !no-underline hover:!no-underline 
                {{ request()->is('admin/properties*') ? 'bg-blue-100 text-blue-600' : '!text-gray-900 hover:bg-blue-50 hover:!text-blue-600' }}">
                ...
                class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition !no-underline hover:!no-underline 
                {{ request()->is('admin/payments*') ? 'bg-blue-100 text-blue-600' : '!text-gray-900 hover:bg-blue-50 hover:!text-blue-600' }}">
                    <x-heroicon-o-currency-dollar class="w-5 h-5 shrink-0" /> Payments
                </a>

                <a href="/admin/leases/create"
                class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition !no-underline hover:!no-underline 
                {{ request()->is('admin/leases*') ? 'bg-blue-100 text-blue-600' : '!text-gray-900 hover:bg-blue-50 hover:!text-blue-600' }}">
                    <x-heroicon-o-document-text class="w-5 h-5 shrink-0" /> Leases
                </a>

                <a href="/admin/maintenance"
                class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition !no-underline hover:!no-underline 
                {{ request()->is('admin/maintenance*') ? 'bg-blue-100 text-blue-600' : '!text-gray-900 hover:bg-blue-50 hover:!text-blue-600' }}">
        ...
        <div class="px-2 py-2">
            <div class="flex items-center gap-3 mb-3">
                <div class="flex items-center justify-center bg-gray-200 rounded-full w-10 h-10 shrink-0">
